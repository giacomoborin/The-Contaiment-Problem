...
Let $ R $ be a regular ring containing a field e let $ I $ be an ideal with big height $ h $, for all intgers $ n \geq 1 $, $ s \geq 0 $ and $ 0 \leq s_1 \leq  ... \leq s_n $ such that $ s = s_1 +  ... + s_n $ we have:
\begin{equation}\label{eq:cont:bighgen}
	I^{(s + nh)} \subseteq \prod_{i=1}^{n} I^{(s_i + 1)}
\end{equation}
\end{teo}
