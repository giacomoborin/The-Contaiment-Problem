...
\item Non mi è chiaro perchè l'altezza dell'ideale $ I_{X_C} $ sia $ n $, ho due possibili motivi:
\begin{enumerate}
...
\begin{tboxtodo}
Qui voglio inserire alcune referneze, senza entrare troppo nel dettaglio della regolaità, direi di usare Eisenbud (\cite{Eisenbud95})
\end{tboxtodo}
...
In our case we consider v-star configuartions in $ \PP^n $ defined by the general hyperplanes in $ \PP^n $ $ \HH  = \{ H_1 , ... , H_v\}$, with $ H_i $ associated to the linear form $ l_i $ (a linear map from $ \PP^n $ to the field of scalars $ k $). Given an $ n $-subset of $ V $\footnote{Since $ V $ is finite we can index it using natual numbers and assume $ V = \{ 1 , ... , v\} $} $ \sigma : = \{ \sigma_1 , ... , \sigma_v\}$ we can associate to it a point of the configuaration $ P_{\HH, \sigma }= \cap_{\sigma_i \in \sigma} H_{\sigma_i}$, that has as vanishing ideal $ I_{P_{\HH, \sigma}} = \left< l_{\sigma_1} , ... , l_{\sigma_v} \right\rangle $.\\
...
\alpha (I_{X_	C})+1=\reg(I_{X_C}) = v-n +1 & \text{ if } t < n-1
\end{cases}
\end{equation}
\end{prop}
