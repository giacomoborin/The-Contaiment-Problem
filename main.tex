...
since for each $ \beta \in B $ the ideal $ \p_\beta $ has $ |\beta | \geq \tau $ elements, we have $ \mm_V \in \p_\beta $ (it is enought to use $\tau$ of the $ |b| $ variables and complete with the missing one). Then using Zarisky-Nagata theorem \ref{teo:zarnaga} we get $ \mm_V \in J^{(\tau)} $.\\
...
\begin{proof}
	From theorem \ref{teo:col:chi} we have $\mm_V^{q-1} \not \in J^q$, therfore we only need to prove $ \mm_V^{q-1} \in J(H)^{( \tau (q-1) )}$ to fail the containment. For this we use as in \ref{teo:col:cont} theorem \ref{teo:zarnaga}:\\
